...
// 追踪要求 service 从 registry 挂载), 工作区刻意建在 HOME 下而非 tmpdir:
// workspace-write 会授权 /tmp 与 os.tmpdir(), tmpdir 下的目录不属于 "外部".
...
beforeEach(() => {
  base = realpathSync(mkdtempSync(join(homedir(), '.dsh-wp-fs-')))
...
describe('WriteProtectFileSystem write/edit 保护', () => {
  ...
  it('workspace-write 下保护目录内的写入被拒绝', async () => {
    ...
    const error = await fs.writeText(target(join(workspace, '.git', 'config')), 'x').then(
      () => undefined,
  ...
  it('指向保护目录内部的符号链接同样被拒绝', async () => {
    await boot('workspace-write', ['.git'])
    mkdirSync(join(workspace, '.git'))
    symlinkSync(join(workspace, '.git'), join(workspace, 'gitlink'))
  ...
  it('danger-full-access 下普通文件可写, 保护路径仍被拒绝', async () => {
    await boot('danger-full-access', ['.git'])
    mkdirSync(join(workspace, '.git'))
  ...
  it('editText 到保护路径在读取内容之前就被拒绝', async () => {
    await boot('workspace-write', ['.git'])
    mkdirSync(join(workspace, '.git'))
    const missing = join(workspace, '.git', 'does-not-exist')
  ...
  it('空保护列表时不产生额外拒绝', async () => {
    ...
    mkdirSync(join(workspace, '.git'))
    await fs.writeText(target(join(workspace, '.git', 'config')), 'x')
    expect(await readFile(join(workspace, '.git', 'config'), 'utf8')).toBe('x')
  })
