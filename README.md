...
- **超大工作区上只有命令侧是有界覆盖**: 家目录级工作区里更深、更靠后 (广度优先队列更晚) 的匹配可能落在同步预算与后台补全预算之外, 此时 Host 日志会告警. write / edit 仍然是完备的 (按模式逐路径判定); 要让**命令**也无条件挡住, 请用锚定条目 (`/.git`) 或按仓库分别声明路径, 别依赖 `**/.git` 一类的全局通配.
...
| `src/patterns.ts` | 把模式**枚举**成具体路径 (有界同步 + 后台分片补全), 供 bash 沙箱与提示词使用 | `node:fs`、`canonicalPath` |
