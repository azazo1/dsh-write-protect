 ...
 * `settingsScope` 绑定 Host 的 {@link PLUGIN_ID} namespace (patterns 字段),
 * 页面保存的文本经 Host settings 持久化并实时生效; base 层是 patch 配置的
 * `readOnlyPaths` 数组, 用户未保存过文本时页面展示 base.
 *
...
/** Host settings namespace 的 patterns 字段解码后的形状. */
...
export function decodeWriteProtectSettings(section: unknown): WriteProtectSettings | undefined {
  ...
  const value = (section as Record<string, unknown>)[PATTERNS_FIELD]
  return typeof value === 'string' ? { patterns: value } : undefined
}
