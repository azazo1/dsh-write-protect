...
export class WriteProtectFileSystem extends SandboxedFileSystem {
  ...
  private async denyIfProtected(displayPath: string, targetKey: string, policy: SandboxExecutionPolicy): Promise<void> {
    ...
    if (typeof patterns === 'string') {
      ...
      if (hit !== undefined) {
        throw new FsError(
          `cannot write "${displayPath}": the path is write-protected by dsh-write-protect (matches "${hit.entry.source}"; the fence is ${hit.path}). Call ${REQUEST_WRITABLE_PATH_TOOL} once for that directory to ask the user for a session grant; one grant covers everything under it.`,
          'FS_SANDBOX_DENIED',
        )
      }
      return
    }
    // 回退: 执行 policy 没有带模式原文, 只能按枚举出来的路径做前缀比较.
