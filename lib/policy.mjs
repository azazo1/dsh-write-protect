		...
		ctx.inject(["systemPrompt"], (scope) => {
			scope.systemPrompt.context({
				...
				text: (context) => {
					...
					if (this.currentLimits().allowWritableRequests) parts.push(`Extra write access is not granted by default: call ${JSON.stringify(REQUEST_WRITABLE_PATH_TOOL)} with a path and a one-sentence justification when a write was denied by write protection or the task needs a path outside the workspace. The user decides in an approval prompt, and the grant lasts only for this session.`);
					else parts.push("Extra write access is not granted by this deployment: do not ask for it, and treat denied writes as final.");
					return parts.join(" ");
				}
			});
		});
