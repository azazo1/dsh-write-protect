import { a as DEFAULT_WRITABLE_PATHS, c as MAX_READONLY_ENTRIES_FIELD, d as READONLY_FILE_FIELD, f as REQUEST_WRITABLE_PATH_TOOL, i as DEFAULT_READ_ONLY_PATHS, l as PATTERNS_FIELD, m as isValidReadonlyFileName, n as ALLOW_REQUESTS_FIELD, o as HARDEN_BROKER_FIELD, p as WRITABLE_FIELD, r as DEFAULT_READONLY_FILE_NAME, s as MAX_GRANTS_FIELD, t as isPathUnder, u as PREVIEW_PATH } from "./containment-CEXstqSf.mjs";
...
var WriteProtectPolicyService = class extends SandboxPolicyService {
	...
	*/
	...
	constructor(ctx, config) {
		...
		this.readOnlyFiles = new ReadOnlyFileCache(200, (message) => this.warn(message));
	...
	*/
	snapshot(workspaceRoot, sessionId) {
		...
		if (now - this.readOnlyCache.at < EXPAND_TTL_MS && this.readOnlyCache.key === readOnlyKey) readOnlySnapshot = this.readOnlyCache;
		else {
			const readOnly = expandReadOnlyPaths(readOnlyPatterns, workspaceRoot);
			for (const warning of readOnly.warnings) this.warn(warning);
			readOnlySnapshot = {
				readOnlyPatterns,
		...
		const writable = expandWritablePaths([...record.extraRoots, writableSettingsText].filter((line) => line.trim().length > 0).join("\n"), workspaceRoot);
		for (const warning of writable.warnings) this.warn(warning);
		return {
