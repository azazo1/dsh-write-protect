import { a as HARDEN_BROKER_FIELD, c as PATTERNS_FIELD, d as READONLY_FILE_FIELD, f as REQUEST_WRITABLE_PATH_TOOL, i as DEFAULT_WRITABLE_PATHS, l as PLUGIN_ID, m as isValidReadonlyFileName, n as DEFAULT_READONLY_FILE_NAME, o as MAX_GRANTS_FIELD, p as WRITABLE_FIELD, r as DEFAULT_READ_ONLY_PATHS, s as MAX_READONLY_ENTRIES_FIELD, t as isPathUnder, u as PREVIEW_PATH } from "./containment-NbNwOc0A.mjs";
...
function stripTrailingSpaces$1(line) {
	let end = line.length;
	while (end > 0 && line[end - 1] === " " && !isEscapedAt$1(line, end - 1)) end -= 1;
	return line.slice(0, end);
}
/** 位置 index 的字符是否被奇数个连续 `\` 转义. */
function isEscapedAt$1(line, index) {
	let slashes = 0;
	for (let i = index - 1; i >= 0 && line[i] === "\\"; i -= 1) slashes += 1;
	return slashes % 2 === 1;
}
/** 按未转义的 `/` 分段, 转义序列原样保留在段内. */
function splitUnescaped$1(line) {
	const segments = [];
	let current = "";
	let i = 0;
	while (i < line.length) {
...
*/
function parsePatternLines$1(text) {
	const entries = [];
	for (const rawLine of text.split(/\r?\n/)) {
		let line = stripTrailingSpaces$1(rawLine);
		if (line.length === 0 || line.startsWith("#")) continue;
		const negated = line.startsWith("!");
		if (negated) line = line.slice(1);
		let dirOnly = false;
		while (line.length > 0 && line.endsWith("/") && !isEscapedAt$1(line, line.length - 1)) {
			line = line.slice(0, -1);
			dirOnly = true;
		}
		...
		const rawSegments = splitUnescaped$1(line);
...
function expandReadOnlyPaths(text, workspaceRoot) {
	const warnings = [];
	const entries = parsePatternLines$1(text);
...
function expandWritablePaths(text, workspaceRoot, options = {}) {
	...
	for (const rawLine of text.split(/\r?\n/)) {
		const line = stripTrailingSpaces$1(rawLine);
		if (line.length === 0 || line.startsWith("#")) continue;
		if (line.startsWith("!")) {
...
* @param entry - 已解析的条目.
*/
function formatPatternEntry(entry) {
	return `${entry.negated ? "!" : ""}${entry.fsAbsolute ? "//" : entry.anchored ? "/" : ""}${entry.segments.join("/")}${entry.dirOnly ? "/" : ""}`;
}
process.platform;
//#endregion
...
*/
function mountPreviewRoute(connection, fallbackRoot, host) {
...
function registerRequestWritablePath(ctx, grants, host) {
	ctx.tools.register(defineTool({
		...
		description: "Ask the user to grant write access to one path for this session. Use it when a write was denied by write protection, or when the task needs a path outside the session workspace. The user decides in an approval prompt, and a granted path stays writable only until the session ends.",
		parameters: {
			path: {
				type: "string",
				required: true,
...
async function askApproval(ctx, grants, policy, sessionId, target, kind, matchedPattern, justification, exec) {
	...
	const notes = kind === "override" ? [`write protection on "${matchedPattern ?? ""}" is bypassed for the write/edit tools under "${target}".`, "sandboxed commands still see the read-only mount or Seatbelt rule on that path; they cannot be widened while the grant is only an in-session decision."] : [`"${target}" joined the writable roots for this session, so sandboxed commands and the write/edit tools may write there.`, "write-protect patterns still win inside it."];
...
var WriteProtectPolicyService = class extends SandboxPolicyService {
	...
	*/
	...
	constructor(ctx, config) {
		...
		ctx.inject(["systemPrompt"], (scope) => {
			scope.systemPrompt.context({
				...
				text: (context) => {
					...
					if (overrides.length > 0) parts.push(`Session write grants that bypass write protection for the write/edit tools (sandboxed commands still see the read-only mount): ${JSON.stringify(overrides)}.`);
		...
		ctx.inject(["connection"], (scope) => {
			...
			scope.effect(() => mountPreviewRoute(connection, this.workspaceRoot, this), "dsh-write-protect: preview route");
		});
	}
	...
	*/
	...
	workspaceRootOfSession(sessionId) {
	...
	*/
	resolve(request = {}) {
		...
		if (sessionId !== void 0) this.rememberSession(sessionId, policy.workspaceRoot);
		return policy;
	}
	/**
