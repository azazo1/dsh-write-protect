import { a as DEFAULT_WATCH_TTL_MIN_MS, c as PREVIEW_PATH, i as DEFAULT_WATCH_TTL_MAX_MS, l as REQUEST_WRITABLE_PATH_TOOL, n as DEFAULT_READONLY_FILE_NAME, o as DEFAULT_WRITABLE_PATHS, r as DEFAULT_READ_ONLY_PATHS, s as GRANTS_PATH, t as isPathUnder, u as isValidReadonlyFileName } from "./containment-CwZXGzMh.mjs";
...
* @returns 一段纯文本.
*/
function grantRevokedText(grant) {
	return [
...
/** 撤回通知在轨迹里的一行说明. */
function grantRevokedSummary(grant) {
...
* @returns 投递结果.
*/
function notifyGrantRevoked(agents, sessionId, grant) {
	const agent = agents?.get(sessionId);
	if (agent === void 0) return "no-session";
	agent.inject(createUserMessage({
		content: [{
			type: "text",
			text: grantRevokedText(grant)
		}],
		source: {
			kind: "write-protect",
			form: "notice",
			summary: grantRevokedSummary(grant)
		}
	}));
	return "queued";
}
...
function jsonError$1(status, message) {
...
function decodeRequest(value) {
	...
	if (typeof sessionId !== "string" || sessionId.trim().length === 0) throw new Error("grants body needs a non-empty sessionId");
	const action = record.action;
	if (action !== "list" && action !== "add" && action !== "revoke") throw new Error("action must be \"list\", \"add\" or \"revoke\"");
	const cwd = record.cwd;
	if (cwd !== void 0 && typeof cwd !== "string") throw new Error("cwd must be a string");
	const path = record.path;
	if (path !== void 0 && typeof path !== "string") throw new Error("path must be a string");
	if (action !== "list" && (path === void 0 || path.trim().length === 0)) throw new Error(`action "${action}" needs a non-empty path`);
	return {
...
*/
function view(host, grants, sessionId, policy, extra = {}) {
	return {
		workspaceRoot: policy.workspaceRoot,
		mode: policy.mode,
		maxGrants: host.maxGrants(),
		grants: grantPreviews(grants.listOf(sessionId)),
		...extra.changed === void 0 ? {} : { changed: extra.changed },
...
*/
async function addGrant(host, grants, body, workspaceRoot) {
	const raw = body.path ?? "";
	...
	if (blocking.length > 0) return jsonError$1(400, `cannot add "${raw}": ${blocking[0]}`);
	if (resolution.path === void 0) return jsonError$1(400, `cannot add "${raw}": give an absolute path, or one relative to the session workspace`);
	const target = resolution.path;
	const rulesFile = host.rulesFilePath(workspaceRoot);
	if (rulesFile !== void 0 && target === rulesFile) return jsonError$1(400, `"${raw}" is the workspace write-protect rules file, which is read-only by design and cannot be granted`);
	const kind = await isPathUnder(target, workspaceRoot) ? "override" : "extra-root";
	const granted = grants.grant(body.sessionId, target, kind);
	if (!granted.ok) return jsonError$1(400, `cannot add "${target}": ${granted.reason}`);
	return Response.json(view(host, grants, body.sessionId, host.resolve(body.sessionId, body.cwd), { changed: {
		path: target,
		kind,
		action: "add"
	} }));
...
* @returns 成功时是响应体, 失败时是错误响应.
*/
function revokeGrant(host, grants, body, workspaceRoot) {
	...
	if (!outcome.ok) return jsonError$1(400, `cannot revoke "${raw}": ${outcome.reason}`);
...
* @param host - 面板需要的插件侧信息.
*/
function mountGrantsRoute(connection, grants, host) {
	const dispose = connection.fetch.register({
		...
		fetch: async (request) => {
			try {
				...
				if (text.length > MAX_BODY_BYTES$1) return jsonError$1(400, "grants body too large");
				const body = decodeRequest(JSON.parse(text));
				const workspaceRoot = host.workspaceRootOfSession(body.sessionId, body.cwd);
				if (workspaceRoot === void 0) return jsonError$1(400, `cannot resolve the workspace root of session "${body.sessionId}"; open that session before managing its write grants (no deployment-root fallback)`);
				...
				return jsonError$1(400, error instanceof Error ? error.message : String(error));
			}
		}
	});
	return () => {
	...
	*/
	...
	constructor(ctx, config) {
		...
		ctx.inject(["connection"], (scope) => {
			...
			scope.effect(() => mountGrantsRoute(connection, this.grants, grantsHost), "dsh-write-protect: grants route");
		});
	}
