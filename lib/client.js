...
function WriteProtectSection(React, props) {
	...
	const savedPatterns = useSyncExternalStore((listener) => scope.subscribe(listener), () => patternsValueOf(scope.getSnapshot().value));
	const savedWritable = useSyncExternalStore((listener) => scope.subscribe(listener), () => writableValueOf(scope.getSnapshot().value));
	...
	const savedWatch = useSyncExternalStore((listener) => scope.subscribe(listener), () => scope.getSnapshot().value?.watchProtectedPaths ?? true);
	const savedWatchMin = useSyncExternalStore((listener) => scope.subscribe(listener), () => scope.getSnapshot().value?.watchTtlMinMs ?? 2e3);
	const savedWatchMax = useSyncExternalStore((listener) => scope.subscribe(listener), () => scope.getSnapshot().value?.watchTtlMaxMs ?? 3e4);
	...
	const [watchDraft, setWatchDraft] = useState(null);
	const [watchMinDraft, setWatchMinDraft] = useState(null);
	const [watchMaxDraft, setWatchMaxDraft] = useState(null);
	...
	})), createElement("p", { className: "dsh-wp-hint" }, "工作区根下这份文件与上面的保护路径同语义, 逐行追加在其后, 因此规则文件既能新增条目也能用 ", createElement("code", null, "!"), " 放行上面的条目. 它只认普通文件 (符号链接被拒绝), ", createElement("code", null, "//"), " 绝对条目与越出工作区的条目会被拒绝, 超过条目上限的部分丢弃并告警. 文件名留空即关闭该识别; 名字里不能有路径分隔符, 也不能用 ", createElement("code", null, ".git"), " 一类元数据名. 这份文件本身永远不可写 (唯一的硬保护): 模型申请可写路径不会放开它, 任何授权都不放行它, 要改只能在这里换文件名或者由你在 DSH 之外编辑.")), createElement("div", { className: "dsh-wp-card" }, createElement("h3", { className: "dsh-wp-card-title" }, "命令侧刷新"), createElement("label", { className: "dsh-wp-toggle" }, createElement("input", {
		type: "checkbox",
		checked: watchValue,
		onChange: (event) => setWatchDraft(event.currentTarget.checked)
	}), createElement("span", null, watchValue ? "监听工作区变化" : "不监听, 只按时间兜底")), createElement("div", { className: "dsh-wp-row" }, createElement("span", { className: "dsh-wp-row-label" }, "刷新下界 (毫秒)"), createElement("input", {
		className: "dsh-wp-input dsh-wp-input-sm",
		inputMode: "numeric",
		value: watchMinValue,
		onChange: (event) => setWatchMinDraft(event.currentTarget.value)
	}), createElement("span", { className: "dsh-wp-row-label" }, "刷新上界 (毫秒)"), createElement("input", {
		className: "dsh-wp-input dsh-wp-input-sm",
		inputMode: "numeric",
		value: watchMaxValue,
		onChange: (event) => setWatchMaxDraft(event.currentTarget.value)
	})), createElement("p", { className: "dsh-wp-hint" }, "命令侧 (bash 等) 的保护路径要展开成真实路径才能交给沙箱, 因此新建的受保护路径可能晚一步进清单. 开启后只给\"正在运行 agent 的会话\"的工作区根装递归监听, 一变就立刻在后台重算, 所以 ", createElement("code", null, "git init"), " 建出来的 ", createElement("code", null, ".git"), " 在后续命令里就会被拦住. 没有事件时按上次展开耗时的 10 倍兜底重算, 并夹在上面两个毫秒值之间. macOS 上的字面条目另有正则拒绝, 不依赖这里. 监听装不上时只告警一次, 退回纯时间兜底.")));
...
function decodeWriteProtectSettings(section) {
	...
	const decoded = {};
	if (Array.isArray(readOnlyPaths) && readOnlyPaths.every((item) => typeof item === "string")) decoded.readOnlyPaths = readOnlyPaths;
	if (Array.isArray(writablePaths) && writablePaths.every((item) => typeof item === "string")) decoded.writablePaths = writablePaths;
	...
	return decoded.readOnlyPaths === void 0 && decoded.writablePaths === void 0 && decoded.patterns === void 0 && decoded.writablePatterns === void 0 && decoded.hardenBroker === void 0 && decoded.readonlyFileName === void 0 && decoded.maxReadOnlyEntries === void 0 && decoded.maxGrants === void 0 && decoded.allowWritableRequests === void 0 && decoded.watchProtectedPaths === void 0 && decoded.watchTtlMinMs === void 0 && decoded.watchTtlMaxMs === void 0 ? void 0 : decoded;
}
...
/** 注册独立配置页. */
function apply(ctx) {
	const scope = ctx.configForms.get(CONFIG_ENTRY_ID);
